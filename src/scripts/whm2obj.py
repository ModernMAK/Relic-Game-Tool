...
from relic.chunky_formats.whm.obj_writer import write_whm
from relic.chunky_formats.whm.whm import WhmChunky


def build_parser():
    parser = argparse.ArgumentParser(prog="WHM 2 OBJ", description="Convert Relic WHM (Model) files to wavefront OBJ files.")
...
def __convert_file(input_file: str, output_file: str, strict: bool = False, quiet: bool = False, indent: int = 0, fmt: str = None, texconv_path: str = None) -> bool:
    ...
    with open(input_file, "rb") as in_handle:
        ...
        write_whm(output_file, whm, out_format=fmt, texconv_path=texconv_path)
    return True
