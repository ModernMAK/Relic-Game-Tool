...
def build_parser():
    parser = argparse.ArgumentParser(prog="RTX 2 Image", description="Convert Relic RTX (Default Texture) files to Images.")
    ...
    parser.add_argument("-s", "--strict", action='store_true', required=False, help="Forces all files provided to be converted. (False by default; both non-'.rtx' and 'non-'Relic Chunky' files are ignored.)")
...
def __convert_file(input_file: str, output_file: str, strict: bool = False, quiet: bool = False, indent: int = 0, out_format: str = None, texconv_path: str = None) -> bool:
    ...
    with open(input_file, "rb") as in_handle:
        ...
        #     x += rtx.shrf.texture.imag.attr.img.extension
        #     output_file = ext
        # with open(output_file, "wb") as out_handle:
        write_rtx(output_file, rtx, out_format=out_format, texconv_path=texconv_path)
        if not quiet:
