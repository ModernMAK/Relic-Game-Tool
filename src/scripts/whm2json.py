...
def build_parser():
    parser = argparse.ArgumentParser(prog="WHM 2 JSON", description="Convert Relic WHM (Model) files to JSON files.")
    ...
    parser.add_argument("-s", "--strict", action='store_true', required=False, help="Forces all files provided to be converted. (False by default; both non-'.whm' and 'non-'Relic Chunky' files are ignored.)")
...
def __convert_file(input_file: str, output_file: str, strict: bool = False, quiet: bool = False, indent: int = 0, fmt: str = None, texconv_path: str = None, fail_on_error:bool=False) -> bool:
    try:
        _, ext = splitext(input_file)
        if ext != ".whm" and not strict:
            return False
        with open(input_file, "rb") as in_handle:
            ...
            with open(output_file+".meshdata.json", "w") as handle:
                write_whm_as_json(handle, whm)  # , out_format=fmt, texconv_path=texconv_path)
        return True
...
def __convert_dir(input_file: str, output_file: str, recursive: bool = False, strict: bool = False, quiet: bool = False, fmt: str = None, texconv_path: str = None, fail_on_error:bool=False):
    for root, folders, files in os.walk(input_file):
        ...
        for file in files:
            ...
            dest = base
            __convert_file(src, dest, strict, quiet, indent=1, fmt=fmt, texconv_path=texconv_path, fail_on_error=fail_on_error)
        ...
        for in_path, out_path in zip(inputs, outputs):
            try:
                if path.isfile(in_path):
                    __convert_file(in_path, out_path, strict, quiet, fmt=fmt, texconv_path=conv, fail_on_error=fail_on_error)
                else:
                    if not quiet:
                        print(f"Reading '{in_path}'...")
                __convert_dir(in_path, out_path, recursive_walk, strict, quiet, fmt=fmt, texconv_path=conv, fail_on_error=fail_on_error)
