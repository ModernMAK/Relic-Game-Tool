...
class FileDefSerializer(StreamSerializer[core.FileDef]):
    ...
    def unpack(self, stream: BinaryIO):
        name_rel_pos, hash_pos, data_rel_pos, length, store_length, modified_seconds,verification_type_val, storage_type_val, crc = self.layout.unpack_stream(stream)
...
class APISerializers(_abc.APISerializer):
    def read(self, stream: BinaryIO, lazy: bool = False, decompress: bool = True) -> Archive:
        ...
        encoded_name: bytes
        encoded_name, header_pos, header_size, data_pos, data_pos, RSV_1, sha_256 = self.layout.unpack_stream(stream)
        if RSV_1 != 1:
