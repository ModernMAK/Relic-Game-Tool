...
from relic.chunky._serializer import ChunkTypeSerializer, chunk_type_serializer
from relic.chunky.errors import ChunkNameError
from relic.chunky.protocols import StreamSerializer
from relic.chunky.v1_1.core import ChunkMeta, RawDataChunk, FolderChunk, Chunky, version
...
class ChunkHeaderSerializer(StreamSerializer[_ChunkHeader]):
    ...
    def unpack(self, stream: BinaryIO) -> _ChunkHeader:
        ...
        chunk_id, version, size, name_size = self.layout.unpack_stream(stream)
        name_buffer = stream.read(name_size)
        try:
            name = name_buffer.rstrip(b"\0").decode("ascii")
        except UnicodeDecodeError as e:
            raise ChunkNameError(name_buffer) from e
        return _ChunkHeader(chunk_type, chunk_id, version, size, name)
    ...
    def pack(self, stream: BinaryIO, packable: _ChunkHeader) -> int:
        ...
        args = packable.chunk_id, packable.version, packable.chunk_type, len(name_buffer)
        written += self.layout.pack(args)
        written += stream.write(name_buffer)
        return written


chunk_header_serializer = ChunkHeaderSerializer(chunk_type_serializer, Struct("<4s 3L"))
...
class RawChunkSerializer(StreamSerializer[AnyRawChunk]):
    ...
    def _pack_data(self, stream: BinaryIO, chunk: RawDataChunk):
        data = chunk.data
        header = _ChunkHeader(chunk.type, chunk.id, chunk.metadata.version, len(data), chunk.metadata.name)
        written = 0
    ...
    def _pack_folder(self, stream: BinaryIO, chunk: FolderChunk):
        ...
        header = _ChunkHeader(chunk.type.value, chunk.id, chunk.metadata.version, 0, chunk.metadata.name)
        written = 0
        written += self.header_serializer.pack(stream, header)
        size = 0
...
api_serializer = APISerializer(version,raw_chunk_serializer)
